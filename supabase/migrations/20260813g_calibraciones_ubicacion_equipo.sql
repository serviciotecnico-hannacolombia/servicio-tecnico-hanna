-- ============================================================
-- Calibraciones: ubicación física del equipo, exclusiva de la coordinación
-- semanal de Sede Hanna Dorado (a diferencia de in situ, aquí el equipo
-- tiene que estar físicamente en la sede antes de que llegue el
-- metrólogo(a) — puede estar ya en sitio, en bodega pendiente de traer, o
-- todavía en mantenimiento).
-- ============================================================

ALTER TABLE ordenes_calibracion ADD COLUMN IF NOT EXISTS ubicacion_equipo text
  CHECK (ubicacion_equipo IN ('en_sitio', 'en_bodega', 'en_mantenimiento'));
